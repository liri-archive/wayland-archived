...
{
public:
    EventReader(EglFSX11Integration *integration)
        : m_integration(integration) { }

    void run();

private:
    EglFSX11Integration *m_integration;
};
...
void EglFSX11Integration::sendConnectionEvent(xcb_atom_t a)
{
...
void EglFSX11Integration::platformInit()
{
...
void EglFSX11Integration::platformDestroy()
{
    running.deref();
...
EGLNativeWindowType EglFSX11Integration::createNativeWindow(QPlatformWindow *platformWindow,
                                                       const QSize &size,
...
void EglFSX11Integration::destroyNativeWindow(EGLNativeWindowType window)
{
    xcb_destroy_window(m_connection, window);
}

bool EglFSX11Integration::hasCapability(QPlatformIntegration::Capability cap) const
{
    Q_UNUSED(cap);
    return false;
}
