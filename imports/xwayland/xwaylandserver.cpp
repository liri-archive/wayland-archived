...
bool XWaylandServer::setup()
{
    ...
    qCInfo(XWAYLAND, "XWayland server listening on display %s",
...
pid_t XWaylandServer::spawn()
{
    ...
    connect(m_process, (void (QProcess::*)())&QProcess::started, [this, server, wm] {
        ...
        ::close(server[1]);
        m_client = wl_client_create(m_compositor->display(), server[0]);
...
void XWaylandServer::shutdown()
{
    ...
    if (!m_lockFileName.isEmpty()) {
        QFile lockFile(m_lockFileName);
        if (!lockFile.remove())
            qCWarning(XWAYLAND) << "Unable to remove" << m_lockFileName;
        lockFile.close();
    }
}
