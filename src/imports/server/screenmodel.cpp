...
void ScreenModel::addFakeScreens()
{
    ...
    if (!file.open(QFile::ReadOnly)) {
        qCWarning(gLcServerQml) << "Could not open configuration file"
    ...
    if (!doc.isObject()) {
        qCWarning(gLcServerQml, "Error parsing \"%s\": no top-level JSON object",
    ...
    for (int i = 0; i < outputs.size(); i++) {
        ...
        qCDebug(gLcServerQml) << "Output settings:" << outputSettings;

        QString name = outputSettings.value(QStringLiteral("name")).toString();
        qCDebug(gLcServerQml) << "Output name:" << name;
        ...
        QPoint pos(x, y);
        qCDebug(gLcServerQml) << "Output position:" << pos;
        ...
        qCDebug(gLcServerQml) << "Output size:" << size;
        qCDebug(gLcServerQml) << "Output refresh rate:" << refreshRate;
        ...
        qCDebug(gLcServerQml) << "Physical size millimiters:" << size;

        Qt::ScreenOrientation orientation =
                static_cast<Qt::ScreenOrientation>(outputSettings.value(QStringLiteral("orientation")).toInt());
        qCDebug(gLcServerQml) << "Output orientation:" << orientation;
