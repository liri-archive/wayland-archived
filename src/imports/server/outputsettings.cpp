...
{
    if (m_initialized) {
        qCWarning(gLcServerQml) << "WaylandOutputSettings cannot change screen after initialization";
        return;
    }
...
{
    if (!m_screen) {
        qCWarning(gLcServerQml) << "WaylandOutputSettings cannot set power state if the screen property is not set";
        return;
    }
