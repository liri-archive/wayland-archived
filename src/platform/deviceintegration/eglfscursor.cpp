      ...
      m_updateRequested(false)
{
    ...
    m_deviceListener = new EglFSCursorDeviceListener(this);
    connect(QGuiApplicationPrivate::inputDeviceManager(), &QInputDeviceManager::deviceListChanged,
            m_deviceListener, &EglFSCursorDeviceListener::onDeviceListChanged);
    updateMouseStatus();
}
...
void EglFSCursor::createShaderPrograms()
{
...
void EglFSCursor::createCursorTexture(uint *texture, const QImage &image)
{
    if (!*texture)
...
#ifndef QT_NO_CURSOR
void EglFSCursor::changeCursor(QCursor *cursor, QWindow *window)
{
    Q_UNUSED(window);
...
bool EglFSCursor::setCurrentCursor(QCursor *cursor)
{
    if (!m_visible)
        return false;
...
bool EglFSCursor::event(QEvent *e)
{
...
void EglFSCursor::update(const QRegion &rgn)
{
...
QRect EglFSCursor::cursorRect() const
{
    return QRect(m_cursor.pos - m_cursor.hotSpot, m_cursor.size);
}

QPoint EglFSCursor::pos() const
{
    return m_cursor.pos;
}

void EglFSCursor::setPos(const QPoint &pos)
{
...
void EglFSCursor::pointerEvent(const QMouseEvent &event)
{
...
void EglFSCursor::draw(const QRectF &r)
{
